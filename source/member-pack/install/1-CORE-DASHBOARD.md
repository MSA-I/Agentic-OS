...
## What you need first
**Node.js, version 22 or newer.** It's free.
- Don't have it? Go to **https://nodejs.org**, click the big green **LTS** button, and run the installer. Click "next" until it's done.
- To check you have it: open **Terminal** (Mac: press ⌘+Space, type "Terminal") and type `node -v`. If it says v22 or higher, you're good.
...
🎉 That's the dashboard. Mission Control is the opening surface, and the existing left sidebar still contains every tool in its familiar order. A tool that needs another dependency says so in its own tab.

On a phone, Agentic OS intentionally becomes a monitoring surface. Use it to read state and perform basic interventions; open the desktop layout for file editing and dense creation work.
...
- **Or native:** follow `25-WINDOWS-INSTALL.md`. In Windows PowerShell set the port with `$env:PORT=3737`, then run `npm run build`, then `npm start`.
