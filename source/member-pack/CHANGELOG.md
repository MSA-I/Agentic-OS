...
## 2026-08-11
- **A complete visual refresh without moving your workspace.** All 47 existing routes, the 244px sidebar, Mission Control sections, agent tabs, cards, and mobile navigation stay in their familiar order.
- **A quieter, accessible visual system.** Perfect White, Outer Space, Warm Red, Geist typography, static tactile relief, visible focus, and reduced-motion behavior replace gradients, glass effects, glow, and decorative idle animation.
- **Agent identities are clearer.** Claude, Codex, Hermes, and OpenClaw keep their existing workflows while their workspaces use the approved provider palettes. Missing OpenMontage demo files now show explicit sample posters instead of broken media requests.
