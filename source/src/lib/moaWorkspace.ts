...
export function activeProfile(): string {
  try {
    const active = fs.readFileSync(path.join(hermesHome(), "active_profile"), "utf8").trim();
    if (active) return active;
  } catch { /* detect profiles below */ }
  try {
    const names = fs.readdirSync(path.join(hermesHome(), "profiles"), { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name);
