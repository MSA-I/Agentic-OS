...
//
// Two roots (the current desktop install uses ~/.gemini/antigravity; older CLI
// builds used ~/.gemini/antigravity-cli):
...
// so all Workspace tabs can be pointed at one real project folder.
const CURRENT_ROOT = path.join(HOME, ".gemini", "antigravity");
