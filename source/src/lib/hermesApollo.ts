...
// The model Hermes is configured to use (so Apollo tracks the same one).
...
export interface ApolloMsg { role: "user" | "assistant"; content: string; }

export interface ApolloResult { ok: boolean; text: string; ms: number; mode: "auto" | "fast" | "agent"; error?: string; }
...
async function minimaxComplete(persona: string, prompt: string, history: ApolloMsg[]): Promise<{ text: string; error?: string }> {
...
export async function complete(persona: string, prompt: string, history: ApolloMsg[]): Promise<{ text: string; error?: string }> {
...
// complete() if the fast model errors. Override the model with AGENTIC_OS_APOLLO_FAST_MODEL.
async function completeFast(persona: string, prompt: string, history: ApolloMsg[]): Promise<{ text: string; error?: string }> {
  const key = readEnv("OPENROUTER_API_KEY");
  if (!key) return complete(persona, prompt, history); // no key → existing path
  const model = readEnv("AGENTIC_OS_APOLLO_FAST_MODEL") || "openai/gpt-4o-mini";
  const messages = [{ role: "system", content: persona }, ...history.slice(-6), { role: "user", content: prompt }];
  const ctrl = new AbortController();
...
async function fast(prompt: string, history: ApolloMsg[]): Promise<ApolloResult> {
...
async function agent(prompt: string, history: ApolloMsg[]): Promise<ApolloResult> {
  const started = Date.now();
...
async function auto(prompt: string, history: ApolloMsg[]): Promise<ApolloResult> {
  const started = Date.now();
...
export async function apolloReply(prompt: string, mode: "auto" | "fast" | "agent", history: ApolloMsg[] = []): Promise<ApolloResult> {
