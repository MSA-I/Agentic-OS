...
function readActiveProfile(): string {
  ...
  const profilesRoot = path.join(HERMES_ROOT, "profiles");
  try {
    const dirs = require("node:fs").readdirSync(profilesRoot, { withFileTypes: true });
    if (dirs.some((d: { name: string; isDirectory(): boolean }) => d.isDirectory() && d.name === "main")) return "main";
    const first = dirs.find((d: { name: string; isDirectory(): boolean }) => d.isDirectory());
    if (first?.name && /^[A-Za-z0-9_.-]+$/.test(first.name)) return first.name;
...
function bucketsForThisMachine(): BucketDef[] {
  const dynamic: BucketDef[] = [];
  const profilesRoot = path.join(HERMES_ROOT, "profiles");
  try {
    const entries = require("node:fs").readdirSync(profilesRoot, { withFileTypes: true });
    for (const entry of entries as { name: string; isDirectory(): boolean }[]) {
      if (!entry.isDirectory() || !/^[A-Za-z0-9_.-]+$/.test(entry.name)) continue;
      dynamic.push({
        id: `profile-${entry.name}`,
        label: `Profile · ${entry.name}`,
        paths: [path.join(profilesRoot, entry.name, "workspace")],
        description: `All workspace files owned by the Hermes ${entry.name} profile.`,
        maxDepth: 4,
      });
    }
  } catch { /* no profiles */ }
  return [...dynamic, ...BUCKETS];
}
...
export async function listBuckets(): Promise<HmProject[]> {
  ...
  const out: HmProject[] = [];
  for (const b of bucketsForThisMachine()) {
...
export async function listBucketFiles(id: string, maxFiles = 200): Promise<{ bucket: HmProject; files: HmFile[] } | null> {
  const def = bucketsForThisMachine().find((b) => b.id === id);
  if (!def) return null;
