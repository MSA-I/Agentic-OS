...
// ── Maple-Preview (DeepGrove) — a SECOND local engine, selectable in the Local UI ──
// 20B-A1B ternary MoE reasoning model on DeepGrove's mlx-lm fork. Lives beside the
// Ollama build engine (it's weak at builds, strong at reasoning) — the Local section
// offers it as a toggle instead of replacing the default engine.
export const MAPLE_BASE = process.env.MAPLE_BASE || "http://127.0.0.1:8124/v1";
// Model path + start script live under ~/maple-mlx by default; override via env.
export const MAPLE_MODEL = process.env.MAPLE_MODEL || path.join(os.homedir(), "maple-mlx", "maple-2bit-mlx");
export const MAPLE_LABEL = "Maple-Preview 20B-A1B · MLX ternary";
const MAPLE_START = process.env.MAPLE_START || path.join(os.homedir(), "maple-mlx", "start.sh");

export async function mapleUp(timeoutMs = 1500): Promise<boolean> {
  try {
    const c = new AbortController();
    const t = setTimeout(() => c.abort(), timeoutMs);
    const r = await fetch(`${MAPLE_BASE}/models`, { cache: "no-store", signal: c.signal });
    clearTimeout(t);
    return r.ok;
  } catch { return false; }
}

// Idempotent: start.sh no-ops if the server is already listening. Cold load is ~15-20s.
export async function ensureMaple(): Promise<boolean> {
  if (await mapleUp()) return true;
  try {
    const { execFile } = await import("node:child_process");
    execFile("/bin/sh", [MAPLE_START], () => {});
  } catch { return false; }
  for (let i = 0; i < 30; i++) {
