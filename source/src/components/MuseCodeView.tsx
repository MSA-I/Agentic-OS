...
const ACCENT = "#0082FB";  // Meta blue
const ACCENT2 = "#a78bfa";  // Muse violet
const HF = "https://openrouter.ai/meta/muse-spark-1.2";
const DOCS = "https://dev.meta.ai/docs/";
const LS_MSGS = "dscoder.msgs";
const LS_SID = "dscoder.sid";
const LS_MODEL = "dscoder.model";
...
function extractCode(msgs: Msg[]): { lang: string; code: string } | null {
  for (let i = msgs.length - 1; i >= 0; i--) {
    ...
    if (!blocks.length) {
      // V4 sometimes returns a bare document with no fences
...
export default function MuseCodeView() {
  const [st, setSt] = useState<Status | null>(null);
  const [msgs, setMsgs] = useState<Msg[]>([]);
  const [input, setInput] = useState("");
  const handleVoice = useVoiceToInput(setInput);
  const [model, setModel] = useState<string>(() => { try { return localStorage.getItem(LS_MODEL) || "meta/muse-spark-1.2"; } catch { return "meta/muse-spark-1.2"; } });
  const [think, setThink] = useState(false);
  useEffect(() => { try { localStorage.setItem(LS_MODEL, model); } catch { /* ignore */ } }, [model]);
  ...
  const refreshSaved = useCallback(() => {
    fetch("/api/musecoder/workspace", { cache: "no-store" }).then((r) => r.json()).then((j) => setSaved({ builds: j.builds || [], sessions: j.sessions || [] })).catch(() => {});
  }, []);

  useEffect(() => {
    const ping = () => fetch("/api/musecoder/status", { cache: "no-store" }).then((r) => r.json()).then(setSt).catch(() => setSt(null));
    ping(); refreshSaved();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch("/api/musecoder/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "saveSession", id: sidRef.current, title, messages: msgs }) }).then(refreshSaved).catch(() => {});
    }, 900);
  ...
  async function send(text?: string) {
    ...
    try {
      const r = await fetch("/api/musecoder/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model, messages: next.map((m) => ({ role: m.role, content: m.content })) }) });
      if (!r.ok || !r.body) {
      ...
      let buf = "", acc = "", replyModel: string | undefined, streamErr: string | null = null;
      for (;;) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (j.thinking) setThink(true);
            if (j.delta) {
              setThink(false);
              acc += j.delta;
              setMsgs((m) => { const c = [...m]; if (c.length && c[c.length - 1].role === "assistant") c[c.length - 1] = { role: "assistant", content: acc, model: replyModel }; return c; });
            }
            if (j.done) replyModel = j.model;
          } catch { /* partial line */ }
        }
      }
      if (streamErr) { setErr(streamErr); setMsgs((m) => m.filter((x, i) => !(i === m.length - 1 && x.role === "assistant" && !x.content))); }
      else setMsgs((m) => { const c = [...m]; if (c.length && c[c.length - 1].role === "assistant") c[c.length - 1] = { role: "assistant", content: acc, model: replyModel }; return c; });
    } catch (e) {
      setThink(false);
  ...
  function download() {
    ...
    a.href = URL.createObjectURL(blob); a.download = `muse-build.${ext}`; a.click();
    URL.revokeObjectURL(a.href);
  }
  async function saveBuild() {
    if (!code) return;
    const title = (msgs.find((m) => m.role === "user")?.content || "build").slice(0, 48);
    await fetch("/api/musecoder/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "saveBuild", code: htmlish ? toDoc(code.code) : code.code, title }) }).catch(() => {});
  ...
  async function loadSession(id: string) {
    const j = await fetch(`/api/musecoder/workspace?session=${encodeURIComponent(id)}`, { cache: "no-store" }).then((r) => r.json()).catch(() => null);
  ...
  return (
    <div className="space-y-4 max-w-[1180px]">
      ...
      {/* HERO */}
      <div className="relative overflow-hidden rounded-2xl border p-4 sm:p-5" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <div className="relative">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.16em] sm:tracking-[0.2em] flex items-center gap-1.5 whitespace-nowrap" style={{ color: ACCENT }}><Sparkles size={11} /> Muse Spark 1.2 · Meta · Live</div>
              <div className="hidden sm:block text-[19px] font-semibold text-[var(--cream)] leading-tight">Muse Code</div>
            </div>
            <div className="w-full order-3 sm:order-none sm:w-auto sm:ml-auto flex items-center gap-2 text-[11px] px-3 py-1.5 rounded-full whitespace-nowrap overflow-hidden text-ellipsis" style={{ background: running ? "rgba(59,130,246,0.12)" : "rgba(255,255,255,0.05)", color: running ? ACCENT : "var(--cream-mute)", border: `1px solid ${running ? ACCENT + "55" : "var(--line-soft)"}`, animation: running ? "hyGlow 3s ease-in-out infinite" : "none" }}>
              ...
              {running ? `${st?.label || "Muse Spark 1.2"} live · OpenRouter` : "No OpenRouter key — see below"}
            </div>
          </div>
          <p className="text-[13px] text-[var(--cream-soft)] mt-3 leading-relaxed max-w-[660px]">
            <b className="text-[var(--cream)]">Muse Spark 1.2</b> — Meta&apos;s coding model, the brain behind their new Muse Code agent, on today&apos;s expanded global access. Type a prompt, watch it build <b className="text-[var(--cream)]">live as it goes</b>. Builds and chat history both survive a refresh.
          </p>
          <div className="flex gap-2 mt-3 overflow-x-auto sm:flex-wrap sm:overflow-visible -mx-1 px-1 pb-1">
            {[{ i: <BadgeCheck size={12} />, t: "co-trained with its harness" }, { i: <Zap size={12} />, t: "built for long-horizon work" }, { i: <Boxes size={12} />, t: "1M context" }, { i: <Cpu size={12} />, t: "reasons before it writes" }].map((c, k) => (
      ...
      {/* WORKSPACE */}
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
        {/* LEFT: chat */}
        <div className="rounded-2xl border flex flex-col overflow-hidden h-[62vh] min-h-[380px] lg:h-[540px]" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)", minHeight: 0 }}>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto scroll p-4 space-y-3">
            {msgs.length === 0 && (
              <div className="h-full grid place-items-center text-center">
                <div>
                  ...
                  <div className="text-[14px] text-[var(--cream)] font-medium">Describe something. Muse Spark builds it.</div>
                  <div className="text-[11px] text-[var(--cream-mute)] mt-1">It previews live as it builds</div>
            ...
            {busy && (
              <div className="flex items-center gap-2 text-[12px]" style={{ color: ACCENT }}>
                ...
                {think ? "Muse Spark is thinking…" : "Muse Spark is building…"} {elapsed}s{elapsed > 8 ? " · V4 reasons before it writes (a full build can take several minutes)" : ""}
              </div>
            )}
          ...
          <div className="p-3 border-t flex items-end gap-2" style={{ borderColor: "var(--line-soft)" }}>
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={1} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder={running ? "Ask Muse Spark to build something…" : "Add OPENROUTER_API_KEY to ~/.hermes/.env"} disabled={!running} className="flex-1 resize-none p-2.5 rounded-lg text-[13px]" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--panel-border)", color: "var(--cream)", opacity: running ? 1 : 0.6 }} />
        ...
        <div className="rounded-2xl border flex flex-col overflow-hidden h-[62vh] min-h-[380px] lg:h-[540px]" style={{ borderColor: "var(--panel-border)", background: "rgba(0,0,0,0.22)", minHeight: 0 }}>
        ...
        <div className="rounded-2xl border p-4" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
          ...
          {saved.builds.length === 0 ? <div className="text-[11.5px] text-[var(--cream-mute)]">Nothing saved yet — build something and hit <b className="text-[var(--cream)]">Save build</b>.</div> : (
            <div className="space-y-1.5 max-h-[220px] overflow-y-auto scroll">
              {saved.builds.map((b) => (
                <a key={b.file} href={`/api/musecoder/workspace?open=${encodeURIComponent(b.file)}`} target="_blank" rel="noopener" className="flex items-center gap-2 px-3 py-2 rounded-lg hy-save" style={{ border: "1px solid var(--line-soft)", background: "rgba(255,255,255,0.02)" }}>
      ...
      <div className="text-[11px] text-[var(--cream-mute)] px-1 flex items-center gap-1.5"><Save size={11} /> Saved to <span className="mono">AGENT-OS-FOLDERS/muse-coder-workspace/</span> — builds as HTML, chats as JSON + readable transcript.</div>
      ...
      <div className="rounded-2xl border px-4 py-3 flex flex-wrap items-center gap-x-4 gap-y-2" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
        <span className="text-[12px] text-[var(--cream-soft)]">Running <b className="text-[var(--cream)]">Muse Spark 1.2</b> — Meta&apos;s coding model on today&apos;s expanded global access, via OpenRouter. 1M-token context; the same brain Muse Code runs.</span>
        <a href={DOCS} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 text-[11.5px]" style={{ color: ACCENT }}><ExternalLink size={12} /> Muse Code docs</a>
        <a href={HF} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 text-[11.5px]" style={{ color: ACCENT }}><ExternalLink size={12} /> OpenRouter</a>
      </div>
    </div>
  );
}
