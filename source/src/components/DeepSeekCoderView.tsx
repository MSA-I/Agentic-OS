...
const ACCENT = "#4d6bfe";   // DeepSeek blue
const ACCENT2 = "#22d3ee";  // cyan
const HF = "https://huggingface.co/tencent";
const DOCS = "https://api-docs.deepseek.com/";
...
export default function DeepSeekCoderView() {
  const [st, setSt] = useState<Status | null>(null);
  const [msgs, setMsgs] = useState<Msg[]>([]);
  const [input, setInput] = useState("");
  const handleVoice = useVoiceToInput(setInput);
  const [model, setModel] = useState<string>(() => { try { return localStorage.getItem(LS_MODEL) || "deepseek-v4-flash"; } catch { return "deepseek-v4-flash"; } });
  ...
  const refreshSaved = useCallback(() => {
    fetch("/api/dscoder/workspace", { cache: "no-store" }).then((r) => r.json()).then((j) => setSaved({ builds: j.builds || [], sessions: j.sessions || [] })).catch(() => {});
  }, []);

  useEffect(() => {
    const ping = () => fetch("/api/dscoder/status", { cache: "no-store" }).then((r) => r.json()).then(setSt).catch(() => setSt(null));
    ping(); refreshSaved();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch("/api/dscoder/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "saveSession", id: sidRef.current, title, messages: msgs }) }).then(refreshSaved).catch(() => {});
    }, 900);
  ...
  async function send(text?: string) {
    ...
    try {
      const r = await fetch("/api/dscoder/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model, messages: next.map((m) => ({ role: m.role, content: m.content })) }) });
      if (!r.ok || !r.body) {
  ...
  function download() {
    ...
    a.href = URL.createObjectURL(blob); a.download = `deepseek-build.${ext}`; a.click();
    URL.revokeObjectURL(a.href);
  }
  async function saveBuild() {
    if (!code) return;
    const title = (msgs.find((m) => m.role === "user")?.content || "build").slice(0, 48);
    await fetch("/api/dscoder/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "saveBuild", code: htmlish ? toDoc(code.code) : code.code, title }) }).catch(() => {});
  ...
  async function loadSession(id: string) {
    const j = await fetch(`/api/dscoder/workspace?session=${encodeURIComponent(id)}`, { cache: "no-store" }).then((r) => r.json()).catch(() => null);
  ...
  return (
    <div className="space-y-4 max-w-[1180px]">
      ...
      <div className="relative overflow-hidden rounded-2xl border p-4 sm:p-5" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <div className="relative">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.16em] sm:tracking-[0.2em] flex items-center gap-1.5 whitespace-nowrap" style={{ color: ACCENT }}><Sparkles size={11} /> DeepSeek V4 · Official API · Live</div>
              <div className="hidden sm:block text-[19px] font-semibold text-[var(--cream)] leading-tight">DeepSeek Coder</div>
            </div>
            <div className="w-full order-3 sm:order-none sm:w-auto sm:ml-auto flex items-center gap-2 text-[11px] px-3 py-1.5 rounded-full whitespace-nowrap overflow-hidden text-ellipsis" style={{ background: running ? "rgba(59,130,246,0.12)" : "rgba(255,255,255,0.05)", color: running ? ACCENT : "var(--cream-mute)", border: `1px solid ${running ? ACCENT + "55" : "var(--line-soft)"}`, animation: running ? "hyGlow 3s ease-in-out infinite" : "none" }}>
              ...
              {running ? `${st?.label || "DeepSeek V4"} live · official API` : "No DeepSeek key — see below"}
            </div>
          </div>
          <p className="text-[13px] text-[var(--cream-soft)] mt-3 leading-relaxed max-w-[660px]">
            <b className="text-[var(--cream)]">DeepSeek V4</b>, straight off DeepSeek&apos;s own API — the 0731 build with the agent upgrade. Type a prompt, watch it build <b className="text-[var(--cream)]">live as it goes</b>. Builds and chat history both survive a refresh.
          </p>
          <div className="flex gap-2 mt-3 overflow-x-auto sm:flex-wrap sm:overflow-visible -mx-1 px-1 pb-1">
            {[{ i: <BadgeCheck size={12} />, t: "official API · dated build" }, { i: <Zap size={12} />, t: "agent-tuned (0731)" }, { i: <Boxes size={12} />, t: "1M context" }, { i: <Cpu size={12} />, t: "reasons before it writes" }].map((c, k) => (
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
        ...
        <div className="rounded-2xl border flex flex-col overflow-hidden h-[62vh] min-h-[380px] lg:h-[540px]" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)", minHeight: 0 }}>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto scroll p-4 space-y-3">
            {msgs.length === 0 && (
              <div className="h-full grid place-items-center text-center">
                <div>
                  ...
                  <div className="text-[14px] text-[var(--cream)] font-medium">Describe something. DeepSeek builds it.</div>
            ...
            {busy && (
              <div className="flex items-center gap-2 text-[12px]" style={{ color: ACCENT }}>
                ...
                {think ? "DeepSeek is thinking…" : "DeepSeek is building…"} {elapsed}s{elapsed > 8 ? " · V4 reasons before it writes (a full build can take several minutes)" : ""}
              </div>
            )}
          ...
          <div className="p-3 border-t flex items-end gap-2" style={{ borderColor: "var(--line-soft)" }}>
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={1} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder={running ? "Ask DeepSeek to build something…" : "Add DEEPSEEK_API_KEY to ~/.fcc/.env"} disabled={!running} className="flex-1 resize-none p-2.5 rounded-lg text-[13px]" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--panel-border)", color: "var(--cream)", opacity: running ? 1 : 0.6 }} />
        ...
        <div className="rounded-2xl border p-4" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
          ...
          {saved.builds.length === 0 ? <div className="text-[11.5px] text-[var(--cream-mute)]">Nothing saved yet — build something and hit <b className="text-[var(--cream)]">Save build</b>.</div> : (
            <div className="space-y-1.5 max-h-[220px] overflow-y-auto scroll">
              {saved.builds.map((b) => (
                <a key={b.file} href={`/api/dscoder/workspace?open=${encodeURIComponent(b.file)}`} target="_blank" rel="noopener" className="flex items-center gap-2 px-3 py-2 rounded-lg hy-save" style={{ border: "1px solid var(--line-soft)", background: "rgba(255,255,255,0.02)" }}>
      ...
      <div className="rounded-2xl border px-4 py-3 flex flex-wrap items-center gap-x-4 gap-y-2" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
        <span className="text-[12px] text-[var(--cream-soft)]">Running <b className="text-[var(--cream)]">DeepSeek V4</b> on DeepSeek&apos;s own API — the endpoint the V4-Flash-0731 agent upgrade shipped on, so you know you&apos;re on the current build. 1M-token context.</span>
        <a href={DOCS} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 text-[11.5px]" style={{ color: ACCENT }}><ExternalLink size={12} /> DeepSeek API docs</a>
