...
type Tab = "chat" | "history" | "workspace";
          ...
          <aside className="space-y-2 min-w-0">
            <div className="flex items-center justify-between mb-1">
              <div className="text-[10px] uppercase tracking-widest text-[var(--fg-dimmer)] flex items-center gap-1.5">
                <FolderOpen size={11} /> Projects &amp; conversations
              </div>
            ...
            <div className="text-[11px] text-[var(--fg-dim)] mb-2">
              Projects and conversation artifacts are shown separately. Reads <code>~/.gemini/antigravity/</code> with a legacy CLI fallback.
            </div>
            <div className="scroll max-h-[60vh] overflow-y-auto space-y-1.5 pr-1">
              ...
              {(["scratch", "brain"] as const).map((kind) => {
                const rows = projects.filter((p) => p.kind === kind);
                return (
                  <section key={kind} className="space-y-1.5">
                    <div className="px-1 pt-2 text-[9px] uppercase tracking-[0.18em] text-[var(--fg-dimmer)]">
                      {kind === "scratch" ? `Projects (${rows.length})` : `Conversations (${rows.length})`}
                    </div>
                    {rows.length === 0 && <div className="px-1 py-2 text-[11px] text-[var(--fg-dimmer)]">None found.</div>}
                    {rows.map((p) => {
                      const active = selected?.root === p.root;
                      return <motion.button key={p.root}
              ...
              <>
                <div className="panel p-3">
                  ...
                  <div className="text-[10.5px] text-[var(--fg-dimmer)] font-[var(--font-geist-mono)] break-all">
                    {collapseHomePath(selected.root)}
                  </div>
                </div>
