...
export default function HiggsfieldView() {
  ...
  const handleVoice = useVoiceToInput(setPrompt);
      ...
      <div className="relative overflow-hidden rounded-2xl border p-4 sm:p-5" style={{ borderColor: "var(--panel-border)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <div className="relative">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.16em] sm:tracking-[0.2em] flex items-center gap-1.5 whitespace-nowrap" style={{ color: ACCENT }}><Sparkles size={11} /> Higgsfield · MCP · Studio</div>
              <div className="hidden sm:block text-[19px] font-semibold text-[var(--cream)] leading-tight">Higgsfield</div>
            </div>
            <div className="w-full order-3 sm:order-none sm:w-auto sm:ml-auto flex items-center gap-2 text-[11px] px-3 py-1.5 rounded-full whitespace-nowrap overflow-hidden text-ellipsis" style={{ background: live ? "rgba(192,132,252,0.12)" : "rgba(255,255,255,0.05)", color: live ? ACCENT : "var(--cream-mute)", border: `1px solid ${live ? ACCENT + "55" : "var(--line-soft)"}` }}>
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: "rgba(240,171,252,0.35)", background: "rgba(192,132,252,0.06)" }}>
          ...
          <div className="mt-2 flex items-center gap-2">
            <code className="flex-1 px-3 py-2 rounded-lg text-[12px] mono" style={{ background: "rgba(0,0,0,0.35)", border: "1px solid var(--line-soft)", color: "var(--cream)" }}>
              {st?.loginCommand || "hermes -p main mcp login higgsfield"}
            </code>
            <button
              onClick={() => { navigator.clipboard.writeText(st?.loginCommand || "hermes -p main mcp login higgsfield"); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
