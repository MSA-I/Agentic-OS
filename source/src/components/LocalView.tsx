...
// De-LaTeX a reply so chat doesn't fill with \boxed{}/\(...\) soup (Maple's habit),
// then let ReactMarkdown handle the rest.
function cleanMath(t: string): string {
  return t
    .replace(/\\boxed\{([^}]*)\}/g, "**$1**")
    .replace(/\\\(([\s\S]+?)\\\)/g, "`$1`")
    .replace(/\\\[([\s\S]+?)\\\]/g, "\n\n`$1`\n\n")
...
// Styles live HERE (not only globals.css) so a long-running dev server picks them up
// with the component's own HMR — the globals pipeline was serving a stale chunk.
const MD_CSS = `
.local-md > *:first-child { margin-top: 0; }
.local-md > *:last-child { margin-bottom: 0; }
.local-md p { margin: 0.45em 0; }
.local-md h1, .local-md h2, .local-md h3, .local-md h4 { font-size: 1em; font-weight: 700; color: var(--cream); margin: 0.7em 0 0.3em; }
.local-md ul, .local-md ol { margin: 0.4em 0; padding-left: 1.25em; }
.local-md li { margin: 0.18em 0; }
.local-md code { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 0.88em; background: rgba(0,0,0,0.35); border: 1px solid var(--line-soft); border-radius: 5px; padding: 0.1em 0.35em; color: #e6c69a; }
.local-md pre { background: rgba(0,0,0,0.4); border: 1px solid var(--line-soft); border-radius: 10px; padding: 10px 12px; overflow-x: auto; margin: 0.5em 0; }
.local-md pre code { background: none; border: 0; padding: 0; color: #bae6fd; }
.local-md strong { color: var(--cream); }
.local-md table { border-collapse: collapse; margin: 0.5em 0; font-size: 0.92em; }
.local-md th, .local-md td { border: 1px solid var(--line-soft); padding: 4px 9px; }
.local-md blockquote { border-left: 2px solid var(--line-soft); margin: 0.5em 0; padding-left: 0.8em; color: var(--cream-mute); }
.local-md hr { border: 0; border-top: 1px solid var(--line-soft); margin: 0.7em 0; }
`;

function Prose({ text }: { text: string }) {
  return (
    <div className="local-md">
      <style>{MD_CSS}</style>
      <ReactMarkdown remarkPlugins={[remarkGfm]}>{cleanMath(text)}</ReactMarkdown>
    </div>
  );
}

const ACCENT = "#5eead4"; // local teal/mint
const MAPLE_ACCENT = "#e6c69a"; // maple gold
...
const ENGINE_KEY = "agentic-os/local/engine/v1";

type Engine = "auto" | "maple";
...
export default function LocalView() {
  ...
  const [partialThink, setPartialThink] = useState("");
  ...
  useEffect(() => {
    try {
      ...
      const e = localStorage.getItem(ENGINE_KEY); if (e === "maple") setEngine("maple");
    } catch {}
    setReady(true);
  }, []);
  useEffect(() => { if (ready) try { localStorage.setItem(ENGINE_KEY, engine); } catch {} }, [engine, ready]);
  ...
  useEffect(() => {
    fetch(`/api/local/model${engine === "maple" ? "?engine=maple" : ""}`)
      .then((r) => r.json()).then((d) => { if (d?.model) setModelName(d.model); }).catch(() => {});
  }, [engine]);
  ...
  const send = useCallback(async (override?: string) => {
    ...
    setMsgs(next); setInput(""); setStreaming(true); setPartial(""); setPartialThink("");
    const ctrl = new AbortController(); ctrlRef.current = ctrl;
    let acc = "", think = "", errMsg: string | null = null, tps: number | undefined;
    try {
      const r = await fetch("/api/local/chat", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ prompt: text, engine, history: next.slice(0, -1).map((m) => ({ role: m.role, text: m.text })) }),
        signal: ctrl.signal,
      });
      if (r.body) {
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              // Maple streams its reasoning chain separately — show it dim while live,
              // keep it OUT of the final saved reply (it's scratch work, not the answer).
              else if (j.t === "think") { think += j.c; setPartialThink(think); }
    ...
    if (!acc.trim() && think.trim() && !errMsg) {
      // reasoning ate the whole budget with no final answer (known Maple failure mode) — say so honestly
      errMsg = "Maple thought past its token budget without landing on an answer — try a simpler ask or switch engines.";
    }
    ...
    setPartial(""); setPartialThink(""); setStreaming(false);
  }, [input, streaming, msgs, engine]);
      ...
      <div className="flex items-center gap-3 mb-3 shrink-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[10.5px] text-[var(--cream-mute)] mt-1 flex items-center gap-1.5">
            ...
            {lastTps != null && <span className="inline-flex items-center gap-0.5" style={{ color: engine === "maple" ? MAPLE_ACCENT : ACCENT }}><Zap size={10} /> {lastTps} tok/s</span>}
          </div>
          {/* engine toggle — the build engine (Ollama) vs the reasoning engine (Maple on :8124) */}
          <div className="mt-1.5 inline-flex items-center gap-1 p-0.5 rounded-lg border border-[var(--line-soft)] bg-[var(--bg-mid)]">
            <button onClick={() => setEngine("auto")} disabled={streaming}
              className="px-2 py-0.5 rounded-md text-[10px] font-semibold transition"
              style={engine === "auto" ? { background: ACCENT, color: "#06201b" } : { color: "var(--cream-mute)" }}
              title="Ollama build engine — follows whatever's warm (gemma4:12b)">⚒ Builder</button>
            <button onClick={() => setEngine("maple")} disabled={streaming}
              className="px-2 py-0.5 rounded-md text-[10px] font-semibold transition"
              style={engine === "maple" ? { background: MAPLE_ACCENT, color: "#241a08" } : { color: "var(--cream-mute)" }}
              title="Maple-Preview 20B (DeepGrove, MLX :8124) — reasoning specialist, auto-starts on first use">🍁 Maple · reasoning</button>
          </div>
        </div>
        {/* tabs */}
              ...
              return (
                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className="max-w-[82%]">
                    {(prose || m.role === "user") && (
                      <div className={`rounded-xl px-3.5 py-2.5 text-[13.5px] leading-relaxed ${m.role === "user" ? "whitespace-pre-wrap" : ""}`}
                        style={m.role === "user"
                        ...
                        {m.role === "user" ? (prose || "…") : <Prose text={prose || "…"} />}
                      </div>
                    )}
                    {art && (
            ...
            {streaming && (
              <div className="flex justify-start">
                <div className="max-w-[82%] space-y-1.5">
                  {partialThink && (
                    <div className="rounded-xl px-3.5 py-2.5 text-[12px] leading-relaxed whitespace-pre-wrap opacity-70"
                      style={{ background: "rgba(230,198,154,0.06)", border: `1px dashed ${MAPLE_ACCENT}40`, color: "var(--cream-mute)", maxHeight: 180, overflowY: "auto" }}>
                      <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest mb-1" style={{ color: MAPLE_ACCENT }}>🍁 thinking</span>
                      <div>{partialThink.slice(-1500)}</div>
                    </div>
                  )}
                  <div className="rounded-xl px-3.5 py-2.5 text-[13.5px] leading-relaxed" style={{ background: "var(--bg-card)", border: "1px solid var(--line-soft)", color: "var(--cream-soft)" }}>
                    {stripHtml(partial) ? <Prose text={stripHtml(partial)} /> : (partial ? <span className="inline-flex items-center gap-2 text-[var(--cream-mute)]"><Wand2 size={13} className="animate-pulse" style={{ color: ACCENT }} /> writing the build…</span> : <span className="inline-flex items-center gap-2 text-[var(--cream-mute)]"><Loader2 size={13} className="animate-spin" style={{ color: engine === "maple" ? MAPLE_ACCENT : ACCENT }} /> {engine === "maple" ? (partialThink ? "Maple is reasoning…" : "waking the oracle (first use loads the model, ~20s)…") : "thinking locally…"}</span>)}
                  </div>
                </div>
              </div>
            )}
