...
// in the same workspace + gallery as the text Apollo, so a voice "build a snake
...
interface Line { id: string; role: "you" | "apollo"; text: string; }
...
export default function ApolloRealtime({ voice = "ash", onClose }: { voice?: string; onClose?: () => void }) {
  ...
  const [hist, setHist] = useState<Array<{ id: string; you: string; apollo: string; ts: number; kind?: string }>>([]);
  ...
  // Persist each turn to the same history store the rest of Apollo uses (disk + Obsidian vault).
  function logTurn(you: string, apollo: string) {
    if (!you && !apollo) return;
    fetch("/api/hermes/apollo-log", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ you, apollo, kind: "realtime" }) }).catch(() => {});
  }
  ...
  const asstDelta = (delta: string) => { curAsstTextRef.current += delta; setLines((l) => {
    ...
    const id = nid(); curAsstRef.current = id; return [...l, { id, role: "apollo", text: delta }];
  }); };
  ...
  function finalizeResponse(ev: { response?: { output?: Array<{ content?: Array<{ transcript?: string; text?: string }> }> } }) {
    ...
    if (!text) {
      ...
      text = text.trim();
      if (text) setLines((l) => [...l, { id: nid(), role: "apollo", text }]);
    }
  ...
  async function handleFunctionCall(ev: { call_id?: string; name?: string; arguments?: string }) {
    ...
    if (target) setLines((l) => [...l, { id: nid(), role: "apollo", text: `⚙️ Opening ${target}…` }]);
    let ok = false;
  ...
  // Build a single HTML app on-device via the same Agent Factory the text Apollo uses,
  // stream it, preview it live, and report back so APOLLO confirms aloud.
  async function runBuild(prompt: string, call_id?: string) {
    ...
    setLines((l) => [...l, { id: nid(), role: "apollo", text: `⚙️ Building “${prompt}”…` }]);
    ...
    if (file) {
      setPreviewFile(file);
      setLines((l) => [...l, { id: nid(), role: "apollo", text: "✅ Built — previewing below." }]);
      sendFnResult(call_id, { success: true, file });
    } else {
      setLines((l) => [...l, { id: nid(), role: "apollo", text: `⚠️ Couldn't build that${err ? ` — ${err}` : ""}.` }]);
  ...
  async function sdpExchange(model: string, ephemeral: string, offerSdp: string): Promise<string> {
    // OpenAI's realtime /calls endpoint burst-rate-limits (429) — retry with backoff
    // before giving up, and surface the REAL failure instead of a generic message.
    const url = `https://api.openai.com/v1/realtime/calls?model=${model}`;
    const headers = { Authorization: `Bearer ${ephemeral}`, "Content-Type": "application/sdp" };
    let lastErr = "";
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        const r = await fetch(url, { method: "POST", body: offerSdp, headers });
        if (r.ok) return await r.text();
        const body = await r.text().catch(() => "");
        if (r.status === 429) {
          if (/insufficient_quota/i.test(body)) {
            // Out of credits — retrying is pointless, say it plainly.
            throw new Error("OpenAI account is out of credits — top up billing at platform.openai.com to use realtime voice.");
          }
          lastErr = "OpenAI rate-limited the realtime call — retrying…";
          await new Promise((res) => setTimeout(res, 1500 * (attempt + 1)));
          continue;
        }
        if (r.status >= 500) {
          // OpenAI-side blip — verified transient (same handshake succeeds seconds later)
          lastErr = `OpenAI realtime had a server error (HTTP ${r.status}) — retrying…`;
          await new Promise((res) => setTimeout(res, 1500 * (attempt + 1)));
          continue;
        }
        try { lastErr = JSON.parse(body)?.error?.message || `HTTP ${r.status}`; } catch { lastErr = `HTTP ${r.status}`; }
        break; // non-429 error — retrying won't help
      } catch (e) { lastErr = String((e as Error).message || e); await new Promise((res) => setTimeout(res, 1000)); }
    }
    throw new Error(lastErr || "Couldn't open the realtime audio link.");
  }
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      pc.onconnectionstatechange = () => {
        ...
        if (cs === "failed" || cs === "closed") setStatus((s) => (s === "live" ? "idle" : s));
        // A real network drop (laptop sleep, wifi blip) — not an intentional close — self-heals back to realtime.
        if (cs === "failed" && startedRef.current && !document.hidden) { cleanup(); setTimeout(() => { if (!document.hidden) void connectRef.current(); }, 800); }
      };
  ...
  // Keep connectRef pointing at the latest connect() so the drop/visibility handlers never go stale.
  useEffect(() => { connectRef.current = connect; }, [connect]);

  useEffect(() => {
    const onVis = () => { if (document.hidden) disconnect(); else if (startedRef.current) void connectRef.current(); };
  ...
  // If the page loaded without a user gesture, the browser may block Apollo's audio.
  ...
  async function loadHistory() {
    try { const d = await fetch("/api/hermes/apollo-log", { cache: "no-store" }).then((r) => r.json()); setHist(d.turns || []); } catch { /* ignore */ }
    setShowHist(true);
  }
    ...
    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: `${CYAN}55`, background: "rgba(34,211,238,0.04)" }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b" style={{ borderColor: `${CYAN}33` }}>
        <span className="text-[12px] font-mono flex items-center gap-2" style={{ color: CYAN }}>
          ...
          REALTIME · {status === "live" ? (speaking ? "APOLLO SPEAKING" : "LISTENING — just talk") : status.toUpperCase()}
        </span>
          ...
          hist.length === 0 ? <div className="text-[12.5px] text-[var(--fg-dimmer)] text-center py-6">No saved conversations yet.</div> : (
            hist.map((h) => (
              <div key={h.id} className="border-b border-[var(--panel-border)]/40 pb-2 mb-1.5">
                ...
                {h.apollo && <div className="text-[13.5px] leading-relaxed"><span className="text-[10px] uppercase tracking-widest mr-2" style={{ color: CYAN }}>APOLLO</span>{h.apollo}</div>}
              </div>
            ))
          )
        ) : (
          <>
            ...
            {lines.map((l) => (
              <div key={l.id} className="text-[14px] leading-relaxed">
                <span className="text-[10px] uppercase tracking-widest mr-2" style={{ color: l.role === "you" ? "var(--gold)" : CYAN }}>{l.role === "you" ? "You" : "APOLLO"}</span>
      ...
      {(building || previewFile) && (
        <div className="border-t" style={{ borderColor: `${CYAN}33` }}>
          ...
          {previewFile && !building && (
            <iframe key={previewFile} src={jPreviewUrl(previewFile)} title="apollo voice build" className="w-full border-0 bg-black" style={{ height: 320 }} sandbox="allow-scripts allow-pointer-lock allow-same-origin" />
          )}
        </div>
      )}
