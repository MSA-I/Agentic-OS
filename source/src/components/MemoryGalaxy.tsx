...
export default function MemoryGalaxy({ onOpenNote }: Props) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      neb(w * 0.10, h * 0.75 - drift, Math.max(w, h) * 0.24, "rgba(52,211,153,0.045)");
    ...
    const loop = () => {
      ...
      if (!reducedMotion && cam.autoRotate && dragRef.current.kind === null) { cam.yaw += 0.0009; cam.pitch = -0.18 + Math.sin(Date.now() * 0.00006) * 0.06; }
      render();
      animRef.current = requestAnimationFrame(loop);
    };
    const setAnimationState = (state: "running" | "reduced-motion" | "paused-hidden" | "stopped") => {
      canvas.dataset.animationState = state;
    };
    const startLoop = () => {
      if (running || document.hidden) return;
      running = true;
      setAnimationState(reducedMotion ? "reduced-motion" : "running");
      animRef.current = requestAnimationFrame(loop);
    };
    const stopLoop = (state: "paused-hidden" | "stopped" = "stopped") => {
      running = false;
    ...
    const onVisibilityChange = () => {
      if (document.hidden) stopLoop("paused-hidden");
      else startLoop();
    };
    const onMotionChange = (event: MediaQueryListEvent) => {
      reducedMotion = event.matches;
      if (reducedMotion) camRef.current.autoRotate = false;
      setAnimationState(reducedMotion ? "reduced-motion" : "running");
      render();
    };
    if (reducedMotion) camRef.current.autoRotate = false;
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (d.kind === "rotate") {
        ...
        canvas.dataset.orbit = `${camRef.current.yaw.toFixed(3)},${camRef.current.pitch.toFixed(3)}`;
    ...
    const onWheel = (e: WheelEvent) => {
      e.preventDefault();
      camRef.current.targetDistance = Math.max(150, Math.min(2600, camRef.current.targetDistance * Math.pow(1.0015, e.deltaY)));
      canvas.dataset.zoom = camRef.current.targetDistance.toFixed(2);
    };
    const onDouble = () => { if (!reducedMotion) camRef.current.autoRotate = !camRef.current.autoRotate; };
  ...
  return (
    <div className="absolute inset-0" ref={wrapRef} data-memory-galaxy data-memory-stars={stats?.nodes ?? 0} data-memory-links={stats?.links ?? 0}>
      <canvas
        ref={canvasRef}
        className="absolute inset-0 block"
        role="img"
        aria-label={`Memory Galaxy with ${stats?.nodes ?? 0} stars and ${stats?.links ?? 0} links. Use the Clean graph option for keyboard navigation.`}
      />
