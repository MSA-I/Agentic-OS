...
import {
  ...
  Search, TrendingUp, Loader2, ArrowRight, Link2, Crosshair,
} from "lucide-react";
...
interface ResearchTopic {
  ...
  site?: string; // set when analysing "all sites"
}
interface ResearchResult {
  ...
  sites?: string[]; skipped?: string[]; // present on the "all sites" combined view
}

// Opportunity filters for the Research tab. "noclick" = the user's ask: keywords with
// zero clicks but real impressions (traffic you're shown for but nobody clicks).
type ResearchFilter = "all" | "noclick" | "striking" | "lowctr" | "gap";
const RESEARCH_FILTERS: { key: ResearchFilter; label: string }[] = [
  { key: "all", label: "All" },
  { key: "noclick", label: "No clicks · high impressions" },
  { key: "striking", label: "Striking distance" },
  { key: "lowctr", label: "Low CTR" },
  { key: "gap", label: "Content gap" },
];
function matchesFilter(t: ResearchTopic, f: ResearchFilter): boolean {
  if (f === "all") return true;
  if (f === "noclick") return t.clicks === 0 && t.impressions >= 50;
  if (f === "striking") return t.badges.includes("STRIKING DISTANCE");
  if (f === "lowctr") return t.badges.includes("LOW CTR");
  if (f === "gap") return t.badges.includes("CONTENT GAP") || t.badges.includes("HIGH IMPRESSIONS");
  return true;
}
...
export default function SEOView() {
  ...
  const [rFilter, setRFilter] = useState<ResearchFilter>("all");

  // ── OpenSEO (embedded local tool, runs in Docker on :3001) ─────────
  const [openseoUp, setOpenseoUp] = useState<boolean | null>(null);
  const [officeUp, setOfficeUp] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    let stop = false;
    const ping = () => fetch("/api/seooffice/status", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => { if (!stop) setOfficeUp(!!j.running); })
      .catch(() => { if (!stop) setOfficeUp(false); });
    ping();
    const id = setInterval(ping, 5000);
    return () => { stop = true; clearInterval(id); };
  }, [tab]);
    ...
    <div className="space-y-5">
          ...
          <div className="panel p-4 flex items-center gap-3 flex-wrap">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11.5px] text-[var(--fg-dim)] mt-0.5">
                {rConnected !== false ? <>Pulling live, read-only search analytics from your verified GSC properties. Optional: set <code className="metric">SERPAPI_KEY</code> for richer competitor SERP data.</> : <>Run <code className="metric">python3 scripts/gsc-connect.py</code> once to connect your own Search Console (read-only). See <code className="metric">install/36-SEO-RESEARCH.md</code>.</>}
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <select value={rSite} onChange={(e) => setRSite(e.target.value)} className="mt-1 w-full bg-[rgba(0,0,0,0.25)] border border-[var(--panel-border)] rounded-lg px-3 h-[38px] text-sm outline-none focus:border-[var(--panel-border-hot)] text-[var(--fg)]">
                  ...
                  {rSites.length > 1 && <option value="all">🌐 All sites (combined)</option>}
                  {rSites.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
                {rSite === "all"
                  ? <p className="text-[10px] text-[var(--fg-dimmer)] mt-1 metric">every connected GSC property, merged</p>
          ...
          {rData && (
            <div className="space-y-3">
              <div className="flex items-center gap-x-4 gap-y-1 flex-wrap text-[11.5px] text-[var(--fg-dim)] px-1">
                ...
                {rData.site === "all" && <span className="text-lime-300/90">🌐 combined from {rData.sites?.length ?? 0} sites{rData.skipped?.length ? ` · ${rData.skipped.length} skipped` : ""}</span>}
              </div>
              ...
              {/* opportunity filters — incl. zero-click / high-impression keywords */}
              {rData.topics.length > 0 && (
                <div className="flex items-center gap-1.5 flex-wrap px-1">
                  {RESEARCH_FILTERS.map((f) => {
                    const n = rData.topics.filter((t) => matchesFilter(t, f.key)).length;
                    const on = rFilter === f.key;
                    return (
                      <button key={f.key} onClick={() => setRFilter(f.key)}
                        className={`text-[11px] px-2.5 h-[26px] rounded-full border transition ${on ? "border-[rgba(163,230,53,0.5)] bg-[rgba(163,230,53,0.16)] text-lime-300" : "border-[var(--panel-border)] text-[var(--fg-dim)] hover:text-[var(--fg)]"}`}>
                        {f.label} <span className="metric opacity-70">{n}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              {(() => {
                const base = rData.topics.filter((t) => matchesFilter(t, rFilter));
                const filtered = rFilter === "noclick" ? [...base].sort((a, b) => b.impressions - a.impressions) : base;
                if (rData.topics.length > 0 && filtered.length === 0) return <div className="panel p-5 text-center text-[12.5px] text-[var(--fg-dim)]">No keywords match this filter in this window — try another filter or a longer date range.</div>;
                return filtered.map((t) => (
                <div key={(t.site || "") + t.keyword} className="panel p-4">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        {t.site && <span className="text-[10px] px-1.5 py-0.5 rounded-full border border-[var(--panel-border)] text-[var(--fg-dim)] metric">{t.site}</span>}
      ...
      {tab === "office" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="text-[13px] text-[var(--fg-dim)] max-w-[60ch]">
              <span className="text-[var(--fg)] font-medium">SEO Office</span> — a local-first SEO agency OS: 3D office UI, claude-seo specialist agents, and a marketing brain. Runs on this Mac from <span className="metric">~/seo-office</span>. AGPL-licensed — cloned from GitHub, not bundled in the member zip.
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <span className="flex items-center gap-1.5 text-[11.5px] metric px-2.5 py-1 rounded-full border"
                style={{
                  borderColor: officeUp ? "rgba(163,230,53,0.4)" : "var(--panel-border)",
                  color: officeUp ? "#a3e635" : "var(--fg-dim)",
                }}>
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: officeUp ? "#a3e635" : officeUp === false ? "#f87171" : "#a1a1aa" }} />
                {officeUp == null ? "checking…" : officeUp ? "running" : "not running"}
              </span>
              <a href="http://localhost:3000/office" target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[12.5px] transition border-[var(--panel-border)] text-[var(--fg-dim)] hover:text-[var(--fg)] hover:border-[rgba(163,230,53,0.4)]">
                <ExternalLink size={14} /> Open full
              </a>
            </div>
          ...
          {officeUp === false ? (
            <div className="rounded-xl border border-[var(--panel-border)] p-6 text-[13px] text-[var(--fg-dim)] leading-relaxed">
              <div className="flex items-center gap-2 text-amber-300 mb-2"><AlertCircle size={16} /> SEO Office isn&apos;t responding on localhost:3000.</div>
              Start it in a terminal:
              <pre className="mt-2 p-3 rounded-lg bg-[rgba(255,255,255,0.04)] text-[12px] overflow-x-auto">cd ~/seo-office &amp;&amp; pnpm dev</pre>
              First time here? Clone it first: <span className="metric">git clone https://github.com/AgriciDaniel/seo-os ~/seo-office &amp;&amp; cd ~/seo-office &amp;&amp; pnpm install</span>
            </div>
          ) : (
            <div className="rounded-xl overflow-hidden border border-[var(--panel-border)]" style={{ height: "78vh", background: "#0b0b10" }}>
              <iframe src="http://localhost:3000/office" title="SEO Office" className="w-full h-full border-0" allow="clipboard-write" />
            </div>
          )}
        </div>
      )}
