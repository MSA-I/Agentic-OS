...
export default function Sidebar() {
  ...
  return (
    <aside data-agent-os-sidebar className="hidden md:flex flex-col w-[244px] shrink-0 h-screen overflow-hidden py-6 border-r border-[var(--line-soft)]"
...
export function MobileNav() {
  ...
  useEffect(() => {
    ...
    const closeAtDesktop = (event: MediaQueryListEvent) => {
      if (event.matches) setOpen(null);
    };
    if (desktopQuery.matches) setOpen(null);
  ...
  useEffect(() => {
    ...
    const backgroundElements = [
      document.querySelector<HTMLElement>("[data-agent-os-main]"),
      document.querySelector<HTMLElement>("[data-agent-os-sidebar]"),
      document.querySelector<HTMLElement>("[data-agent-os-mobile-nav]"),
  ...
  return (
    <>
      {open && (
        <div
          id="agent-os-mobile-menu"
          ref={dialogRef}
          data-agent-os-mobile-menu
          role="dialog"
          aria-modal="true"
          aria-labelledby="agent-os-mobile-menu-title"
          tabIndex={-1}
        ...
        >
          <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-[var(--line-soft)]">
            <span id="agent-os-mobile-menu-title" className="text-[11px] uppercase tracking-[0.2em] text-[var(--fg-dimmer)]">{open === "agents" ? "Choose your agent" : "Agentic OS · all sections"}</span>
      ...
      <nav data-agent-os-mobile-nav className="md:hidden !fixed bottom-0 left-0 right-0 z-40 border-t border-[var(--line-soft)]"
        ...
        <div className="flex items-stretch justify-around px-1 py-1.5">
          {MOBILE_PINNED.map((href) => {
            ...
            return (
              <Link key={href} href={href} aria-current={active ? "page" : undefined}
                ...
                style={{ color: active ? "var(--fg)" : "var(--fg-dim)", background: active ? item.dim : "transparent" }}>
                {item.icon}
          ...
          <button onClick={() => setOpen("agents")} aria-label="Choose an agent"
            aria-controls="agent-os-mobile-menu" aria-expanded={open === "agents"}
            className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg min-w-[56px]"
            style={{
          ...
          <button onClick={() => setOpen("all")} aria-label="Open full menu"
            aria-controls="agent-os-mobile-menu" aria-expanded={open === "all"}
            className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg min-w-[56px]"
