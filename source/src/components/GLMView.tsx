...
const BUCKET = "glm"; // reuse the Hermes "GLM 5.2 ✦" workspace bucket
...
export default function GLMView() {
  ...
  const send = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/glm/chat", {
  ...
  function clearChat() { if (confirm("Clear GLM chat history?")) { setMsgs([]); try { localStorage.removeItem(HISTORY_KEY); } catch {} } }

  // ── workspace (reuses the Hermes "glm" bucket) ──
        ...
        <div className="panel flex flex-col min-h-0 flex-1 p-0 overflow-hidden">
            ...
            {msgs.length === 0 && !streaming && (
              <div className="h-full grid place-items-center text-center">
                <div>
                  ...
                  <div className="text-[13.5px] text-[var(--cream)]">Chat with GLM-5.2.</div>
                  <div className="text-[11.5px] text-[var(--cream-mute)] mt-1">Zhipu&apos;s flagship coder on your Coding Plan. Ask it to build, fix, or explain anything.</div>
                </div>
              </div>
            )}
            {msgs.map((m, i) => (
            ...
            {streaming && (
              <div className="flex justify-start">
                <div className="max-w-[82%] rounded-xl px-3.5 py-2.5 text-[13.5px] leading-relaxed whitespace-pre-wrap" style={{ background: "var(--bg-card)", border: "1px solid var(--line-soft)", color: "var(--cream-soft)" }}>
                  {partial || <span className="inline-flex items-center gap-2 text-[var(--cream-mute)]"><Loader2 size={13} className="animate-spin" style={{ color: ACCENT }} /> GLM-5.2 is thinking…</span>}
                </div>
              </div>
            )}
          ...
          <div className="border-t border-[var(--line-soft)] p-3 flex items-end gap-2 shrink-0">
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={2}
              ...
              placeholder="Ask GLM-5.2 to build something, fix code, or answer a question…  (⌘+Enter to send)"
            ...
            {streaming
              ...
              : <button onClick={send} disabled={!input.trim()} className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold disabled:opacity-40" style={{ background: ACCENT, color: "#062019" }}><Send size={14} /> Send</button>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-3 flex-1 min-h-0">
          ...
          <div className="panel p-2 flex flex-col min-h-0">
            <div className="flex items-center justify-between px-2 py-1.5">
              <div className="text-[10px] uppercase tracking-[0.25em] text-[var(--cream-mute)] font-semibold flex items-center gap-1.5"><FileCode size={12} /> GLM 5.2 builds</div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto scroll space-y-1">
              {files.length === 0 && <div className="text-[11px] text-[var(--cream-mute)] px-2.5 py-3">No builds yet. Ask GLM-5.2 in Chat to build something — it lands here.</div>}
              {files.map((f) => (
          ...
          <div className="panel p-0 flex flex-col min-h-0 overflow-hidden">
            {!openFile ? (
              ...
              <>
                ...
                <div className="flex-1 min-h-0 overflow-auto bg-[#06120f]">
