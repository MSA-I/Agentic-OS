...
export default function SakanaView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/sakana/history", { cache: "no-store" });
  ...
  useEffect(() => {
    ...
    fetch("/api/sakana/history", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ msgs: msgs.slice(-200) }) }).catch(() => {});
  }, [msgs]);
  ...
  const send = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/sakana/chat", {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-3 flex-1 min-h-0">
          ...
          <div className="panel p-2 flex flex-col min-h-0">
            <div className="flex items-center justify-between px-2 py-1.5">
              <div className="text-[10px] uppercase tracking-[0.25em] text-[var(--cream-mute)] font-semibold flex items-center gap-1.5"><FileCode size={12} /> Sakana Fugu builds</div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto scroll space-y-1">
              {files.length === 0 && <div className="text-[11px] text-[var(--cream-mute)] px-2.5 py-3">No builds yet. Sakana Fugu HTMLs land at <code>~/.hermes/profiles/sakana-fugu/workspace/</code> and show up here.</div>}
              {files.map((f) => (
