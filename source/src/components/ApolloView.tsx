...
import ApolloBuilds from "./ApolloBuilds";
import ApolloRealtime from "./ApolloRealtime";
...
const VOICES = [
  { id: "ash", label: "Ash (Apollo · butler)" },
...
// Sound effects disabled — Apollo runs silent (no boot sound, no blips or chimes).
function makeSfx() {
  return {
    ensure() {},
    boot() {},
...
function ArcReactor({ phaseRef, levelRef, size }: { phaseRef: React.MutableRefObject<Phase>; levelRef: React.MutableRefObject<number>; size: number }) {
  ...
  const staticColor = phaseColor(phaseRef.current);
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" aria-hidden>
      <circle cx="50" cy="50" r="42" fill="none" stroke={staticColor} strokeOpacity="0.35" strokeDasharray="2 4" />
      <circle cx="50" cy="50" r="31" fill="none" stroke={staticColor} strokeOpacity="0.72" strokeWidth="1.2" />
      <circle cx="50" cy="50" r="17" fill={staticColor} fillOpacity="0.22" stroke={staticColor} strokeWidth="1.4" />
      <path d="M50 34 63 58H37Z" fill="none" stroke="white" strokeOpacity="0.62" />
    </svg>
  );
}
...
function WallBackdrop() {
  ...
  return (
    <div
      ...
      style={{ opacity: 0.1, backgroundImage: 'url("/textures/agent-os/weave.webp")', backgroundSize: "760px" }}
      aria-hidden
    />
  );
}
...
function Waveform({ levelRef }: { levelRef: React.MutableRefObject<number> }) {
  ...
  return (
    <div className="flex w-full items-center justify-center gap-1" style={{ height: 56 }} aria-hidden>
      {Array.from({ length: 28 }, (_, i) => (
        <span
          key={i}
          className="block w-[3px] rounded-sm"
          style={{ height: 8 + ((i * 11) % 30), background: CYAN, opacity: 0.2 + (i % 5) * 0.1 }}
        />
      ))}
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// ── "Show me …" — Apollo paints a live HUD panel when you ask ──
...
// "Apollo, remember …" (save to vault) and "what do you remember about X" (recall from vault).
...
const SHOW_AGENTS = [
  ...
  { n: "Free Claude", c: "#10b981" }, { n: "Apollo", c: "#22d3ee" },
];
...
// "Apollo, brief me" / "give me my weekly briefing" / "what's on my plate today".
...
export default function ApolloView() {
  ...
  const [voice, setVoice] = useState("cedar");   // cedar = new-gen natural male voice (gpt-realtime-2.1)
  ...
  const [telem] = useState({ throughput: 87, latency: 0.4, load: 32, signal: 98 });
  ...
  function logTurn(you: string, apollo: string, kind: string) {
    fetch("/api/hermes/apollo-log", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ you, apollo, kind }) }).catch(() => {});
  }

  // Load the saved transcript on open, so history survives reloads + tab switches.
  useEffect(() => {
    fetch("/api/hermes/apollo-log", { cache: "no-store" })
      .then((r) => r.json())
      .then((j: { turns?: { you: string; apollo: string }[] }) => {
        const loaded: Turn[] = [];
        for (const t of (j.turns || [])) {
          if (t.apollo) loaded.push({ id: ++idRef.current, who: "hermes", text: t.apollo });
  ...
  useEffect(() => {
    const loop = () => {
      ...
      // Snapshot the current state once; the visual layer is intentionally static.
      return;
    };
    loop();
    return;
  ...
  // After Apollo finishes a reply: in LIVE mode, reopen the mic automatically (a
  ...
  // then show the running app live in the Apollo HUD.
  ...
  const ask = useCallback(async (prompt: string) => {
    ...
    if (mem) {
      if (mem.action === "save") {
        fetch("/api/hermes/apollo-memory", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: mem.text }) }).catch(() => {});
    ...
    try { wakeRef.current?.stop(); } catch {}    // don't let Apollo hear itself
    const youId = ++idRef.current; const hermesId = ++idRef.current;
    setTurns((t) => [{ id: hermesId, who: "hermes", text: mode === "agent" ? "On it, sir…" : "…", working: true }, { id: youId, who: "you", text: p }, ...t]);
    setStatus(mode === "agent" ? "APOLLO is acting…" : "APOLLO is thinking…");
    try {
      const history = [...turns].reverse().slice(-6).map((x) => ({ role: (x.who === "you" ? "user" : "assistant") as "user" | "assistant", content: x.text }));
      const r = await fetch("/api/hermes/apollo", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: p, mode, history }) });
      const j = await r.json();
  ...
  // Wake aliases cover how browsers mis-hear "Apollo" (appollo/apolo) + "Hermes".
  function startWake() {
    ...
    if (!rec) {
      ...
      rec.lang = (typeof navigator !== "undefined" && navigator.language) || "en-US";
      rec.onstart = () => { wakeRunningRef.current = true; if (phaseRef.current === "idle" && !busyRef.current) setStatus('Standing by — say "Apollo" or "Hermes".'); };
      rec.onerror = (e) => {
        const err = e?.error || "";
      ...
      rec.onresult = (e) => {
        ...
        const m = low.match(/\b(apollo|appollo|apolo|apollos|hermes|hey apollo|ok apollo)\b/);
        if (m) {
  ...
  const restartWake = safeStartWake;   // speak()/build resume the wake loop after Apollo finishes
  ...
  // loop, push-to-talk) so Apollo isn't recording your conversations or burning
  ...
  useEffect(() => {
    ...
    const onVis = () => {
      if (document.hidden) {
        ...
        if (resumeWake || resumeLive) setStatus("Paused — Apollo stops listening while this tab is in the background.");
      } else {
  ...
  const controls = (
    <div className="flex items-center gap-2 flex-wrap justify-center">
      <button onClick={() => { const n = !realtime; if (n) { if (wakeOnRef.current) stopWake(); if (liveRef.current) toggleLive(); } setRealtime(n); }}
        ...
        <Zap size={13} /> Realtime {realtime ? "ON" : "OFF"}
      </button>
      <button onClick={toggleLive} disabled={realtime} title={realtime ? "Turn Realtime off to use Live" : "Live — hands-free, just talk (no clicks, no wake word)"}
        ...
        <Mic size={13} /> Live {live ? "ON" : "OFF"}
      </button>
      <button onClick={toggleWake} disabled={realtime} title={realtime ? "Turn Realtime off to use the wake word" : 'Wake word — say "Apollo" hands-free'}
        className="px-3 h-9 rounded-lg border text-[12px] flex items-center gap-1.5 transition disabled:opacity-30"
        style={{ borderColor: wake ? CYAN : "var(--panel-border)", color: wake ? CYAN : "var(--fg-dim)", background: wake ? "rgba(34,211,238,0.12)" : "transparent" }}>
        <Radio size={13} /> Wake word {wake ? "ON" : "OFF"}
      </button>
  ...
  const previewPanel = previewFile ? (
    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: `${CYAN}55` }}>
      ...
      <iframe key={previewFile} src={jPreviewUrl(previewFile)} title="apollo build" className="w-full border-0 bg-black" style={{ height: 360 }} sandbox="allow-scripts allow-pointer-lock allow-same-origin" />
    </div>
  ) : null;
    ...
    <div className="max-w-[920px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-medium tracking-tight">Hermes-<span style={{ color: CYAN }}>Apollo</span></h1>
      ...
      {realtime && (
        <div className="mt-4">
          <ApolloRealtime voice={voice} onClose={() => setRealtime(false)} />
        </div>
      )}
      ...
      <div className="space-y-2.5 pb-10">
          ...
          {turns.map((t) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="panel p-3.5">
              <div className="text-[10px] uppercase tracking-widest mb-1.5 flex items-center gap-1.5" style={{ color: t.who === "you" ? "var(--gold)" : CYAN }}>
                {t.who === "you" ? "You" : <><Zap size={10} /> APOLLO</>}
              </div>
        ...
        {turns.length === 0 && (
          <div className="text-center text-[12.5px] text-[var(--fg-dimmer)] leading-relaxed pt-3">
            Try: <span className="text-[var(--fg-dim)]">&ldquo;Apollo, brief me.&rdquo;</span> · <span className="text-[var(--fg-dim)]">&ldquo;Give me my weekly briefing.&rdquo;</span><br />
            <span className="text-[var(--fg-dim)]">&ldquo;Apollo, open YouTube.&rdquo;</span> · <span className="text-[var(--fg-dim)]">&ldquo;Build a snake game in my workspace.&rdquo;</span>
          </div>
        )}
      </div>

      {/* Built with Apollo — preview gallery (a few, with "See more") */}
      <ApolloBuilds initial={6} />
        ...
        {wall && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="relative flex items-start justify-between p-7 font-mono text-[12px]" style={{ color: CYAN }}>
              <div>
                <div className="text-[16px] tracking-[0.3em] mb-1" style={{ color: "var(--fg)" }}>HERMES-<span style={{ color: CYAN }}>APOLLO</span></div>
            ...
            <div className="relative flex-1 grid grid-cols-[1fr_auto_1fr] items-center px-8 min-h-0">
              {/* left readouts — live status + static telemetry bars */}
                ...
                {([["NEURAL THROUGHPUT", telem.throughput, 99], ["CORE LOAD", telem.load, 100], ["SIGNAL", telem.signal, 100]] as [string, number, number][]).map(([k, v, max]) => (
                  <div key={k}>
                    ...
                    <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--line-soft)" }}>
                      <div className="h-full rounded-full" style={{ width: `${(v / max) * 100}%`, background: TEAL }} />
                    </div>
                  </div>
                ))}
                  ...
                  <div className="font-mono text-[12px] space-y-2" style={{ color: "var(--fg-dim)" }}>
                    ...
                    <div className="space-y-2 max-h-[44vh] overflow-hidden">
                      {turns.slice(0, 6).map((t) => (
                        <div key={t.id}><span style={{ color: t.who === "you" ? "var(--gold)" : TEAL }}>{t.who === "you" ? "» YOU  " : "» APOLLO  "}</span><span style={{ color: "var(--fg)" }}>{t.text}</span></div>
                      ))}
