...
export default function ThumbnailStudio() {
  ...
  // Deep-link prefill: /thumbnails?topic=… (used by The Furnace's idea cards).
  useEffect(() => {
    try {
      const t = new URLSearchParams(window.location.search).get("topic");
      if (t && t.trim()) setInstructions(`Thumbnail for this video: ${t.trim().slice(0, 300)}`);
    } catch { /* no-op */ }
  }, []);
  ...
  const handleVoice = useVoiceToInput(setInstructions);
        ...
        <div className="panel p-4 space-y-3.5">
          ...
          <div className="flex items-end gap-2">
            ...
            <textarea
              ...
              rows={4}
              className="flex-1 rounded-lg px-3 py-2 text-[13px] resize-none outline-none"
