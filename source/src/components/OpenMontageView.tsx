...
function SamplePoster({ poster, title }: { poster: string; title: string }) {
  return (
    <div
      className="relative grid w-full place-items-center overflow-hidden bg-[var(--outer-space,#1F2853)]"
      style={{ aspectRatio: "16/9" }}
      role="img"
      aria-label={`${title} sample poster. Preview video is not bundled with this package.`}
    >
      {poster ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={poster} alt="" className="h-full w-full object-cover" />
      ) : (
        <Film size={28} aria-hidden style={{ color: "#FFFFFF" }} />
      )}
      <span
        className="absolute right-2 top-2 rounded px-2 py-1 text-[9px] font-semibold uppercase tracking-widest"
        style={{ background: "#FFFFFF", color: "#101630" }}
      >
        Sample poster
      </span>
    </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        {MOVIES.map((v) => (
          <div key={v.src} className="panel overflow-hidden">
            <SamplePoster poster={v.poster} title={v.title} />
            <div className="px-4 py-3">
