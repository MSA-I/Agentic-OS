...
export default function AgentKanban() {
  ...
  // Deep-link prefill: /agent-kanban?seo=1&goal=… (used by The Furnace's idea cards) —
  // arms SEO-cluster mode and drops the topic into the composer. Applied AFTER the
  // server-board hydration below (which also calls setGoal), so the incoming link wins.
  const applyDeepLink = useCallback(() => {
    try {
      const p = new URLSearchParams(window.location.search);
      const g = p.get("goal");
      if (p.get("seo") === "1") setSeoMode(true);
      if (g && g.trim()) setGoal(g.trim().slice(0, 500));
    } catch { /* no-op */ }
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) { applyDeepLink(); hydrated.current = true; }
    })();
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {/* composer */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-2 mb-3 shrink-0">
        <input value={goal} onChange={(e) => setGoal(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") plan(); }}
          ...
          className="w-full sm:flex-1 bg-[var(--bg-mid)] border border-[var(--line-soft)] rounded-xl px-3.5 py-2.5 text-[13.5px] text-[var(--cream)] placeholder:text-[var(--cream-mute)] focus:outline-none" />
        <button onClick={plan} disabled={!goal.trim() || planning || running} className="inline-flex shrink-0 items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold whitespace-nowrap disabled:opacity-40" style={{ background: "#38bdf8", color: "#04121f" }}>
          {planning ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />} Assemble board
        </button>
        {queuedLeft && !planning && (
          <button onClick={run} disabled={running} className="inline-flex shrink-0 items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold whitespace-nowrap disabled:opacity-40" style={{ background: "#5ab896", color: "#08130d" }}>
