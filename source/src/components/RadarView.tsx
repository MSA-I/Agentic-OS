...
export default function RadarView() {
  ...
  const [briefing, setBriefing] = useState<"idle" | "loading" | "playing">("idle");
  const briefAudioRef = useRef<HTMLAudioElement | null>(null);

  // The Spoken Brief — the oracle reads its top three signals out loud (same TTS the
  // wake word uses). Click again while playing to stop.
  const hearBrief = useCallback(async () => {
    if (briefing === "playing") { briefAudioRef.current?.pause(); briefAudioRef.current = null; setBriefing("idle"); return; }
    const sigs = (viewDay ? history.find((h) => h.day === viewDay)?.signals : live) ?? [];
    if (!sigs.length || briefing === "loading") return;
    setBriefing("loading");
    try {
      const top = sigs.slice(0, 3);
      const text = ["Good morning. Three signals from the oracle.",
        ...top.map((s, i) => `${i + 1}. ${s.headline}. ${String(s.why_now || "").split(/(?<=\.)\s/)[0] || ""} Your angle: ${s.angle}`),
      ].join(" ");
      const r = await fetch("/api/hermes/tts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text }) });
      const j = await r.json();
      if (!j.audio) throw new Error(j.error || "no audio");
      const a = new Audio(j.audio);
      briefAudioRef.current = a;
      a.onended = () => { briefAudioRef.current = null; setBriefing("idle"); };
      setBriefing("playing");
      await a.play();
    } catch { setBriefing("idle"); }
  }, [briefing, live, history, viewDay]);
  ...
  return (
    <div className="rdr">
      <style>{`
        ...
        .rdr-heat .fill{ height:100%; border-radius:999px; background:#fbbf24; }
      ...
      <div className="rdr-bar">
        ...
        <button className="rdr-trend" onClick={hearBrief} disabled={briefing === "loading"} title="The oracle reads its top three signals out loud" style={briefing !== "idle" ? { borderColor: "#22d3ee", color: "#22d3ee" } : undefined}>
          {briefing === "loading" ? <Loader2 size={14} className="animate-spin" /> : <span>🔊</span>}
          {briefing === "playing" ? "Stop the brief" : briefing === "loading" ? "Summoning…" : "Hear the brief"}
        </button>
      ...
      <div className="rdr-grid">
        ...
        <div className={`rdr-hud ${sweeping ? "sweeping" : ""}`}>
          ...
          <div className="rdr-stage" style={{ ["--oracle" as string]: oracleImg ? `url('${oracleImg}')` : "none" }}>
            ...
            <div className="rdr-orb3d">
              {oracleImg && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={oracleImg} alt="" className="h-full w-full rounded-full object-cover" />
              )}
            </div>
