...
const ACCENT = "#cdd3f7"; // Grok cool silver-blue
const HISTORY_KEY = "agentic-os/grok/history/v1";
const BUCKET = "grok"; // the Hermes "Grok Build ✦" workspace bucket
...
export default function GrokView() {
  ...
  const send = useCallback(async () => {
    ...
    if (tab === "workspace") loadFiles();
  }, [input, streaming, msgs, tab]);

  function stop() { ctrlRef.current?.abort(); setStreaming(false); setPartial(""); }
  function clearChat() { if (confirm("Clear Grok Build chat history?")) { setMsgs([]); try { localStorage.removeItem(HISTORY_KEY); } catch {} } }

  // ── workspace (the Hermes "grok" bucket) ──
        ...
        <div className="panel flex flex-col min-h-0 flex-1 p-0 overflow-hidden">
            ...
            {msgs.length === 0 && !streaming && (
              <div className="h-full grid place-items-center text-center">
                <div>
                  ...
                  <div className="text-[13.5px] text-[var(--cream)]">Chat with Grok Build.</div>
                  <div className="text-[11.5px] text-[var(--cream-mute)] mt-1">xAI&apos;s terminal coding agent, signed in on your plan. Ask it to build something — the file lands in the Workspace tab.</div>
                </div>
              </div>
            )}
            {msgs.map((m, i) => (
            ...
            {streaming && (
              <div className="flex justify-start">
                <div className="max-w-[82%] rounded-xl px-3.5 py-2.5 text-[13.5px] leading-relaxed whitespace-pre-wrap" style={{ background: "var(--bg-card)", border: "1px solid var(--line-soft)", color: "var(--cream-soft)" }}>
                  {partial || <span className="inline-flex items-center gap-2 text-[var(--cream-mute)]"><Loader2 size={13} className="animate-spin" style={{ color: ACCENT }} /> Grok Build is working…</span>}
                </div>
              </div>
            )}
          ...
          <div className="border-t border-[var(--line-soft)] p-3 flex items-end gap-2 shrink-0">
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={2}
              ...
              placeholder="Ask Grok Build to build a game, an app, fix code, or anything…  (⌘+Enter to send)"
            ...
            {streaming
              ...
              : <button onClick={send} disabled={!input.trim()} className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold disabled:opacity-40" style={{ background: ACCENT, color: "#15101a" }}><Send size={14} /> Send</button>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-3 flex-1 min-h-0">
          ...
          <div className="panel p-2 flex flex-col min-h-0">
            <div className="flex items-center justify-between px-2 py-1.5">
              <div className="text-[10px] uppercase tracking-[0.25em] text-[var(--cream-mute)] font-semibold flex items-center gap-1.5"><FileCode size={12} /> Grok Build builds</div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto scroll space-y-1">
              {files.length === 0 && <div className="text-[11px] text-[var(--cream-mute)] px-2.5 py-3">No builds yet. Ask Grok Build in Chat to build something — it lands here.</div>}
              {files.map((f) => (
          ...
          <div className="panel p-0 flex flex-col min-h-0 overflow-hidden">
            {!openFile ? (
              ...
              <>
                ...
                <div className="flex-1 min-h-0 overflow-auto bg-[#0a0a14]">
                  ...
                  {isHtml && previewMode === "preview" && previewUrl && <iframe src={previewUrl} className="w-full h-full border-0 bg-black" title={openFile.name} sandbox="allow-scripts allow-same-origin allow-popups allow-pointer-lock" allow="pointer-lock; fullscreen; gamepad; autoplay" />}
