...
export async function GET() {
  ...
  const body = {
    ...
    openclaw: (() => {
      ...
      // OpenClaw on Windows exits 255 even when perfectly healthy (the health
      // output is complete) — trust the parsed gateway line over the exit code.
      const healthy = openclaw.ok || /Gateway event loop:/.test(openclaw.stdout);
      return {
        ok: healthy,
