...
// Morning push — after a successful sweep, pipe the top three signals to Telegram via
// `hermes send` (no LLM loop; reuses the gateway's bot credentials). Best-effort: a push
// failure must never fail the sweep. Disable with RADAR_TELEGRAM_PUSH=0.
function pushBrief(signals: Signal[]): void {
  if (process.env.RADAR_TELEGRAM_PUSH === "0") return;
  const top = signals.slice(0, 3);
  const lines = [
    "🔮 The oracle spoke — top signals this morning:",
    ...top.map((s, i) => `${i + 1}. ${s.headline} (${s.heat}° · ${s.posted})\n${s.url}`),
    "Full board → http://localhost:3737/hermes (Hermes Oracle)",
  ].join("\n\n");
  void run("hermes", ["-p", "default", "send", "-t", "telegram", lines], { timeoutMs: 30_000 }).catch(() => {});
}
...
async function runSweep(): Promise<void> {
  ...
  try {
    ...
    // Grok's x_search occasionally returns an empty sweep (transient — auth fine, next call
    // works). One empty answer must not kill the 6am run, so retry once before giving up.
    let res = await run("hermes", ["-z", scanPrompt(new Date()), "--provider", RADAR_PROVIDER, "--model", RADAR_MODEL], { cwd: HERMES_WORKSPACE, timeoutMs: 420_000 });
    let raw = res.stdout || "";
    let signals = normalize(extractRaw(raw), raw);
    if (!signals.length && !/not (logged|signed) in|unauthor|no xai|credential/i.test(res.stderr || "")) {
      await writeStatus({ running: true, startedAt, phase: "Empty sweep — consulting the firehose once more…" });
      res = await run("hermes", ["-z", scanPrompt(new Date()), "--provider", RADAR_PROVIDER, "--model", RADAR_MODEL], { cwd: HERMES_WORKSPACE, timeoutMs: 420_000 });
      raw = res.stdout || "";
      signals = normalize(extractRaw(raw), raw);
    }
    if (!signals.length) {
