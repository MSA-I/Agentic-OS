...
export async function POST(req: Request) {
  ...
  if (!resumeSessionId && typeof sessionId === "string" && /^[0-9a-f-]{32,40}$/i.test(sessionId)) {
  ...
  else if (typeof newSessionId === "string" && /^[0-9a-f-]{36}$/i.test(newSessionId)) args.push("--session-id", newSessionId);
  if (!resumeSessionId && typeof sessionName === "string" && sessionName.trim()) args.push("--name", sessionName.trim().slice(0, 80));
