...
import { configuredOmniRouteModelIds, getOpenCodeDefaultModel, OPENCODE_ROOT, resolveOpenCodeInvocation } from "@/lib/opencode";
import { OMNIROUTE_KEY } from "@/lib/omniroute";
...
const BIN_PATH = [
  ...(process.platform === "win32"
    ...
    : [path.join(os.homedir(), ".opencode/bin"), path.join(os.homedir(), ".local/bin"), "/opt/homebrew/bin", "/usr/local/bin"]),
  process.env.PATH || "",
...
export async function POST(req: Request) {
  ...
  const modelId = (typeof model === "string" && /^[\w.\-]+\/[\w.\-:/]+$/.test(model)) ? model : getOpenCodeDefaultModel();
  if (modelId.startsWith("omniroute/") && !configuredOmniRouteModelIds().has(modelId)) {
    return Response.json({ error: "OmniRoute is not configured as an OpenCode provider. Connect it in Setup Center first." }, { status: 409 });
  }
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const invocation = resolveOpenCodeInvocation(args);
      if (!invocation) {
        emit({ type: "error", text: "The configured OpenCode .cmd shim could not be resolved to its package executable." });
        emit({ type: "done", code: 1, project: name });
        controller.close();
        return;
      }
      const child = spawn(invocation.command, invocation.args, {
        cwd,
        env: { ...process.env, PATH: BIN_PATH, NO_COLOR: "1", OMNIROUTE_API_KEY: OMNIROUTE_KEY },
      });
