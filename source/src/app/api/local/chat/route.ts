...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const messages = [
        engine === "maple" ? {
          // Maple is the reasoning specialist, not the build engine — a build-mode prompt
          // just pollutes its answers. Also: it loves LaTeX; ask for plain text so the
          // chat doesn't fill with \boxed{} and \(...\) soup.
          role: "system",
          content:
            "You are Maple-Preview, a reasoning model running entirely on the user's own Mac — offline, free. " +
            "Answer in clean simple Markdown. Write any math in plain text (no LaTeX, no \\boxed, no \\( \\)). " +
            "Keep answers tight: reason privately, then give the answer clearly, with the final result on its own line.",
        } : {
          role: "system",
          content:
      ...
      try {
        // ── Maple-Preview engine (UI toggle) — DeepGrove MLX server on :8124 ──
        // Reasoning specialist beside the Ollama build engine. Auto-starts the server
        // (idempotent start.sh, ~15-20s cold) and streams `reasoning` deltas as
        // {"t":"think"} so the UI can show the chain dim + drop it from the final reply.
        if (engine === "maple") {
          if (!(await mapleUp())) {
            send({ t: "d", c: "" }); // keep the stream alive while the model loads
            const ok = await ensureMaple();
            if (!ok) {
              send({ t: "error", m: "Maple isn't running and auto-start failed — run ~/maple-mlx/start.sh in a terminal, then retry." });
              send({ t: "done" }); controller.close(); return;
            }
          }
          const t0 = Date.now();
          const r = await fetch(`${MAPLE_BASE}/chat/completions`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            // DeepGrove's recommended sampling for Maple: temp 1.0 · top-p 0.95 · top-k 20.
            // Off-spec sampling (0.7) noticeably degrades a ternary model's output quality.
            body: JSON.stringify({ model: MAPLE_MODEL, messages, stream: true, temperature: 1.0, top_p: 0.95, top_k: 20, max_tokens: 16000 }),
          });
          if (!r.ok || !r.body) {
            const t = await r.text().catch(() => "");
            send({ t: "error", m: `Maple server HTTP ${r.status}: ${t.slice(0, 200)}` });
            send({ t: "done" }); controller.close(); return;
          }
          ...
          while (true) {
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (d.reasoning) { send({ t: "think", c: d.reasoning }); toks++; }
