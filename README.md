...
>
> 📦 **This pack: version `2026-08-11` (built 11 August 2026).** To check you're on the latest, compare this against the newest pack in the AI Profit Boardroom — or just run `Update Agent OS.command`.
>
...
## The Agentic OS workspace

The interface still opens on the familiar **Mission Control** screen. The redesign changes the visual language across all 47 screens while keeping the existing routes, APIs, sidebar order, cards, tabs, data, and capabilities in place.

- Claude, Codex, Hermes, and OpenClaw keep their existing workspace structure and tools, with a clearer visual identity for each provider.
- The desktop sidebar remains the primary navigation. Mobile keeps the existing compact bottom navigation and stacked content order.
- Desktop remains the full work environment. Mobile is best suited to monitoring and basic intervention.
- The interface uses a fixed Perfect White, Outer Space, and Warm Red brand system with static tactile relief. There is no theme switch, decorative idle animation, glass, glow, or CSS gradient effect.
- Existing keyboard behavior and deep links are preserved; the redesign does not add a replacement navigation model.

See [`PRODUCT.md`](PRODUCT.md) for the product contract and [`DESIGN.md`](DESIGN.md) for visual tokens, agent themes, responsive behavior, and texture receipts.
...
| 41 | `41-MUSE-CODE.md` | Meta's Muse Spark 1.2 coding model (OpenRouter key) | Optional |
...
| **Muse Code** | Meta's Muse Spark 1.2 — agent-tuned builds, 1M context | OpenRouter key → `41-MUSE-CODE.md` |
...
### ✨ New in this version
- **Hermes Muse works on YOUR channel** — the daily YouTube idea forge now reads the channel you set in config (`youtubeChannel` / `furnaceChannels`) instead of anyone else's, and tells you exactly what to set if it's blank → `install/4-HERMES.md`.
- **New tab: Muse Code** — Meta's **Muse Spark 1.2** agent-tuned coding model (1M context) via your OpenRouter key → `install/41-MUSE-CODE.md`.
...
Built as a local-first control plane:

- **Agentic OS itself runs on YOUR computer.** Local state is not uploaded to an Agentic OS service. A configured external AI, search, voice, image, video, or publishing provider receives data only when you invoke the workflow that uses it.
