...
**Mission Control is the home screen — a hub.** Every tab in the existing sidebar remains its own tool, with its own model, local CLI, or operational purpose. They do **not** all run through one big pipeline, and you can use any tab on its own.

Think of it like an office building: Mission Control is the lobby, and each sidebar tab is a different room with a different specialist in it. The visual redesign repaints that building; it does not move the rooms or change their order.
