...
**Or in the dashboard:** the build version shows under the "Agentic OS" logo (top-left), so you can confirm you're on the latest pack. Agent tabs stay in their familiar sidebar positions; an unconfigured tool simply reports that it still needs setup.
...
2. **Log in once** the normal way for that tool. Its tab in the dashboard lights up when it finds the tool and your login.
